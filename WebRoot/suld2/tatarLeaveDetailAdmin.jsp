...
.service{ height:50px; padding-top:11px; box-sizing:border-box; position:relative; box-sizing:border-box; background:#fff;}
...
/*底部按钮*/
.evaluate_btn,.share_btn{ background-color:#FF7F00; color:white; border-radius:3px; width:44%; height:33px; line-height:33px; text-align:center; font-size:16px; margin-left:4%; float:left}
...
<body style="background-color: #f9f9f9;">
 <form class="form form-horizontal" action="<%=request.getContextPath()%>/suld_/suld.do" method="post"  id="tatarForm" name="tatarForm">
   <input type="hidden" name="method" id="method" value="moibleLeaveSubmitAdmin"/>
...
<div class="waiter_box">
    ...
    <div class="waiter">
        ...
        <h2><%=bean.getUNITNAME()==null?"":bean.getUNITNAME() %>
        <!-- 
        ...
        <p><%=bean.getDepartment()==null?"":bean.getDepartment()%></p>
    </div>
...
<div class="order_box">
    ...
    <div class="order_msg">
        ...
        <div class="msg_line">
            <h1>开始时间</h1>
            <p><%=bean.getSTARTTIME() %>&nbsp;<%=bean.getDAYTYPEID()==null?"":bean.getDAYTYPEID() %></p>
            <div class="clearfix"></div>
        </div>
	 <div class="msg_line">
            <h1>结束时间</h1>
            <p><%=bean.getENDTIME() %> &nbsp;<%=bean.getDAYTYPE()==null?"":bean.getDAYTYPE() %></p>
            <div class="clearfix"></div>
        </div>
        <div class="msg_line">
            <h1>备注内容</h1>
...
<%if(bean.getFIRSTAPPROVALSTATE().equals("待审批")&&bean.getFIRSTAPPROVALPERSON().equals(webUser.getSid().toString()+"-"+webUser.getUserName())){ %>
	<div class="evaluate_btn" onClick="init(1,1)">通过</div>
    <div class="share_btn"  onClick="init(1,2)">不通过</div>

<%} %>
<%if(bean.getSECONDAPPROVALSTATE().equals("待审批")&&bean.getFIRSTAPPROVALSTATE().equals("已通过")&&bean.getSECONDAPPROVALPERSON().equals(webUser.getSid().toString()+"-"+webUser.getUserName())){ %>
	<div class="evaluate_btn" onClick="init(2,1)">通过</div>
    <div class="share_btn"  onClick="init(2,2)">不通过</div>

<%} %>
<%if(bean.getTHREEAPPROVALSTATE().equals("待审批")&&bean.getSECONDAPPROVALSTATE().equals("已通过")&&bean.getTHREEAPPROVALPERSON().equals(webUser.getSid().toString()+"-"+webUser.getUserName())){ %>
	<div class="evaluate_btn" onClick="init(3,1)">通过</div>
    <div class="share_btn"  onClick="init(3,2)">不通过</div>
...
$("#submitSucc").click(function(){
	$("#tatarForm").submit();
});
$("#submitFal").click(function(){
	$("#sid").val(a);	
	$("#tatarForm").submit();
});
